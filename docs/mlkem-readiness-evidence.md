...
result, one Android emulator benchmark result, and one Windows hosted-CI
benchmark result are recorded as partial evidence in
...
`benchmarks/release-device-results.android-emulator.2026-06-04.json` and
hosted-CI Windows evidence in
`benchmarks/release-device-results.windows-github-actions.2026-06-04.json`.
This is still a production blocker for fallback providers because the
release-device matrix is not complete across all supported fallback platforms
...
| Windows | x64 and ARM64 where available | keygen, encapsulation, decapsulation, malformed rejection, allocations, p50/p95/p99 | Partial, hosted CI only |
